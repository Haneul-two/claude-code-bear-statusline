...
Claude Code용 곰 상태표시줄. **곰의 표정(감정)**, 모델명, 세션 비용, 5시간 세션 한도(초기화 **시각**), 주간 한도(초기화 **날짜**)를 한 줄로 보여줍니다.

A bear-themed status line for Claude Code — a bear whose **facial expression reflects your session health**, plus model name, session cost, 5-hour session limit (with reset time) and weekly limit (with reset date) in one line.

```
ʕ◕ᴥ◕ʔ 여유 [Fable 5] | 💰 $1.23 | ⏳ 세션 24% (10:55 초기화) | 📅 주간 81% (6/18(목) 초기화)
```
...
| 항목 | 의미 |
|---|---|
| `ʕ◕ᴥ◕ʔ 여유` | 곰의 감정 — 세션 상태에 따라 표정이 변합니다 (아래 표 참고) |
...
- **표정 변경**: 스크립트의 곰 카오모지(`ʕ•ᴥ•ʔ` 등)를 원하는 얼굴로 교체
